...
Biar gak penuh, ada di sini ya brow: <https://rpubs.com/ZenR_Prog/MSR-Prak-08>

# P Value - Uji Hipotesis

> Jika data diasumsikan tidak normal bagaimana ukuran sampel berpengaruh kesalahan tipe 1 pada uji hipotesis?

Contoh kasus sebaran:
...
install_load("ggplot2","lmtest","mvtnorm","MBESS","dplyr","DT")
```
...
```{r}
t_test_p_value <- function(x) t.test(x)$p.value
```

> **Uji Wilcoxon**.

```{r}
wilcox_test_p_value <- function(x) wilcox.test(x)$p.value
```

> **Simulasi**.

```{r}
test_case <- function(sample_size, case_number, test_function) {
  p_values <- replicate(1000, { # 1000 ulangan
    data <- generate_data(sample_size, case_number)
    test_function(data)
  })
  mean(p_values)
}
...
```{r}
generate_data <- function(n, case_number) {
  normal_size <- n %/% 2
  chi_size <- n - normal_size
  
  # Data Kasus 1
  if (case_number == 1) {
    normal_data <- rnorm(normal_size, mean = 0, sd = 1)
  ...
  # Data Kasus 3    
    ...
    chi_size <- n %/% 4
    normal_size <- n - 3 * chi_size
    chi_squared_1_data <- rchisq(chi_size, df = 1)
    chi_squared_3_data <- rchisq(chi_size, df = 3)
    normal_1_data <- rnorm(normal_size, mean = 0, sd = 1)
...
plot_data_t_test <- data.frame(
  ...
  P_Value = c(
    sapply(n, test_case, case = 1, test = t_test_p_value), 
    sapply(n, test_case, case = 2, test = t_test_p_value), 
    sapply(n, test_case, case = 3, test = t_test_p_value)
  ),
...
plot_data_wilcox_test <- data.frame(
  ...
  # Menghitung p-value untuk setiap ukuran sampel
  P_Value = c(
    sapply(n, test_case, case = 1, test = wilcox_test_p_value), 
    sapply(n, test_case, case = 2, test = wilcox_test_p_value), 
    sapply(n, test_case, case = 3, test = wilcox_test_p_value)
  ),
  Case = rep(c("Kasus Sebaran 1", "Kasus Sebaran 2", "Kasus Sebaran 3"), 
             each = length(sample_sizes))
)
...
```{r}
datatable(plot_data_wilcox_test)
```

# P Value - one-way ANOVA

> Pengaruh banyaknya amatan dan tidak terpenuhinya asumsi sebaran normal terhadap pengujian hipotesis di one-way ANOVA.
