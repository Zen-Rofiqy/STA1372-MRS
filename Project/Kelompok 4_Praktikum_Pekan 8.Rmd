---
title: "Kelompok 4_Praktikum_Pekan 8"
author: "Angga Fathan Rofiqy"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output:
  rmdformats::downcute:
    self_contained: true
    thumbnails: false
    lightbox: true
    gallery: true
    highlight: tango
    code_folding: show
    toc_depth: 3
    number_sections: false
    toc_float:
      collapsed: true
      smooth_scroll: true
    fig_caption: true
pkgdown:
  as_is: true
---

![](images/clipboard-1024985539.png){width="613"}

# Bagian A

## Poin 1 {.tabset}

> Bangkitkan sebaran **Normal**, **Seragam**, dan **Eksponensial** dengan ukuran contoh **2, 5, 25**

Berikut merupakan hasil pembangkitan masing-masing serbaran dengan ukuran contoh 2, 5, 25:

### **Normal**

```{r}
sample_size <- c(2, 5, 25)

norm_samples <- lapply(sample_size, function(n) rnorm(n))

print(norm_samples)
```

Sebaran Normal dengan ukuran contoh (n) **2, 5, 25**

### **Seragam**

```{r}
unif_samples <- lapply(sample_size, function(n) runif(n))

print(unif_samples)
```

Sebaran Seragam dengan ukuran contoh (n) **2, 5, 25**

### **Eksponensial**

```{r}
exp_samples <- lapply(sample_size, function(n) rexp(n))

print(exp_samples)
```

Sebaran Eksponensial dengan ukuran contoh (n) **2, 5, 25**

## Poin 2

> Buat **histogram** dari **rataan contoh**nya

```{r}
# Menghitung rata-rata dari masing-masing sampel
norm_means <- sapply(norm_samples, mean)
unif_means <- sapply(unif_samples, mean)
exp_means <- sapply(exp_samples, mean)
```

### **Rata-rata contoh** masing-masing sebaran: {.tabset}

#### **Normal**

```{r}
print(norm_means)
```

#### **Seragam**

```{r}
print(unif_means)
```

#### **Eksponensial**

```{r}
print(exp_means)
```

### **Histogram dari rata-rata contoh**

```{r fig.height=3.5, fig.width=10, dpi=300}
par(mfrow = c(1, 3))
hist(norm_means, main = "Histogram Normal Mean", 
     xlab = "Mean", ylab = "Frequency", col = "#8DC16D")
hist(unif_means, main = "Histogram Uniform Mean", 
     xlab = "Mean", ylab = "Frequency", col = "#4493CA")
hist(exp_means, main = "Histogram Exponential Mean", 
     xlab = "Mean", ylab = "Frequency", col = "#D44155")
```

## Poin 3

> Buat **normal qq-plot** dari masing-masing n

```{r fig.height=3.5, fig.width=10, dpi=300}
par(mfrow = c(1, 3))
qqnorm(norm_means, main = "Normal QQ Plot", col = "#8DC16D", lwd = 5,
       xlab = "Theoretical Quantiles", ylab = "Sample Quantiles")
qqline(norm_means, col = "red", lwd=2)

qqnorm(unif_means, main = "Uniform QQ Plot", col = "#4493CA", lwd = 5,
       xlab = "Theoretical Quantiles", ylab = "Sample Quantiles")
qqline(unif_means, col = "red", lwd=2)

qqnorm(exp_means, main = "Exponential QQ Plot", col = "#D44155", lwd = 5,
       xlab = "Theoretical Quantiles", ylab = "Sample Quantiles")
qqline(exp_means, col = "red", lwd=2)
```

# Bagian B

**Bangkitkan dua gugus data**

1.  Data dari sebaran normal
2.  Data campuran:
    -   50% dari sebaran normal + 50% dari sebaran chisquare

    -   50% sebaran chisquare dengan paramenter a + 50% sebaran chisquare dengan paramenter b

    -   25% sebaran chisquare dengan parameter a + 25% sebaran chisuqare dengan parameter b + 25% sebaran normal dengan paramater a + 25% sebaran normal parameter b

```{=html}
<!-- -->
```
3.  Ambil sample dengan ukuran n = 4, 12, 20, 60, 100

```{=html}
<!-- -->
```
4.  Buat histogram dan normal qq-plot

```{=html}
<!-- -->
```
5.  Pada n berapa sebaran rataan dari masing-masing data mulai simeteris atau mendekati sebaran normal?
