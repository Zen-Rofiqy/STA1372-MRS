...
```{r fig.height=20, fig.width=25, dpi=300}
sample_sizes <- c(4, 12, 20, 60, 100)

plot_distribution <- function(data, title, color) {
  df <- data.frame(x = data)
  p1 <- ggplot(data = df, aes(x = x)) +
    geom_histogram(bins = 20, fill = color, alpha = 0.7) +
    geom_density(aes(y = ..count..), fill = color, alpha = 0.4) + # Poligon densitas
...
for (size in sample_sizes) {
  ...
  colors <- c("blue", "green", "orange")  # warna yang berbeda untuk setiap jenis mixed data
