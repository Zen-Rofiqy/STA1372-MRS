...
install_load("fitdistrplus","ggplot2","gridExtra")
```
...
sample_and_plot <- function(data, n) {
  sample_data <- sample(data, n)
  
  hist(sample_data, main = paste("Histogram (n =", n, ")"), 
       xlab = "Value", ylab = "Frequency", col = "#8DC16D")
  
  qqnorm(sample_data, main = paste("Normal QQ Plot (n =", n, ")"), 
         xlab = "Theoretical Quantiles", ylab = "Sample Quantiles")
  qqline(sample_data)
}
...
for (n in sample_sizes) {
  ...
  cat("Data dari sebaran normal:\n")
  sample_and_plot(norm_data, n)
  cat("\n")
  
  cat("Data campuran:\n")
  sample_and_plot(mixed_data_3, n)
  cat("\n\n")
}
```
...
plot_distribution <- function(data, title) {
  ...
  p1 <- ggplot(data = df, aes(x = x)) +
    ...
    geom_density(aes(y = ..count..), fill = "red", alpha = 0.4) + # Poligon densitas
    labs(title = paste("Histogram", title)) +
    theme_minimal()
  
  p2 <- ggplot(data = df, aes(sample = x)) +
    stat_qq() +
    stat_qq_line() +
    labs(title = paste("Normal QQ Plot", title)) +
    theme_minimal()
...
for (size in sample_sizes) {
  ...
  for (title in names(samples)) {
    sample <- sample(samples[[title]], size, replace = FALSE)
    plot_distribution(sample, paste0(title, " (n=", size,")"))
  }
}
```
...
plot_distribution <- function(data, title) {
  ...
  # Mengembalikan list plot
  return(list(p1, p2))
}

# Inisialisasi list untuk menyimpan plot dari setiap iterasi
all_plots <- list()
...
for (size in sample_sizes) {
  ...
  # Inisialisasi list untuk menyimpan plot dari setiap iterasi ukuran sampel
  plots_for_size <- list()
  
  for (title in names(samples)) {
    sample <- sample(samples[[title]], size, replace = FALSE)
    plots <- plot_distribution(sample, paste0(title, " (n=", size,")"))
    plots_for_size <- c(plots_for_size, plots)
  }
  
  # Menggabungkan plot dari setiap iterasi ukuran sampel menjadi satu plot
  all_plots <- c(all_plots, plots_for_size)
}

# Menggabungkan semua plot menjadi satu menggunakan cowplot
library(cowplot)
